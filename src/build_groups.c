...
int build_groups(int Npeaks, double zstop, int first_call)
{
  ...
  */
  ...
  for (this_z = last_z; this_z < nstep; this_z++)
  {
    ...
    skip = 0;
    if (!subbox.pbc[_x_] && (ibox == 0 || ibox == subbox.Lgwbl[_x_] - 1))
      ++skip;
    if (!subbox.pbc[_y_] && (jbox == 0 || jbox == subbox.Lgwbl[_y_] - 1))
      ++skip;
    if (!subbox.pbc[_z_] && (kbox == 0 || kbox == subbox.Lgwbl[_z_] - 1))
      ++skip;
...
void merge_groups(int grp1, int grp2, PRODFLOAT time)
{
  ...
  /* updates zacc of particles in case one of the halos (or both)
     ...
     the threshold */
  if ((groups[grp1].t_appear == -1 || groups[grp2].t_appear == -1) && (groups[grp1].Mass + groups[grp2].Mass >= params.MinHaloMass))
  {
...
#ifdef THREE_LPT
    if (order > 2)
      pos += myobj->w31 * myobj->v31[i] + myobj->w32 * myobj->v32[i];
#endif
#endif
  }
#ifdef RECOMPUTE_DISPLACEMENTS
...
#ifdef THREE_LPT
    vv += myobj->v31[i] * myobj->D31v * myobj->w31 + myobj->v32[i] * myobj->D32v * myobj->w32;
#endif
#endif
  }
#ifdef RECOMPUTE_DISPLACEMENTS
...
#ifdef THREE_LPT
    vv += (myobj->v31_prev[i] * (1. - myobj->w31) + myobj->v31[i] * myobj->w31) * myobj->D31v + (myobj->v32_prev[i] * (1. - myobj->w32) + myobj->v32[i] * myobj->w32) * myobj->D32v;
#endif
#endif
  }
#endif
...
void update(pos_data *obj1, pos_data *obj2)
{
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    else
    {
      ...
      /* checks PBC again */
      if (subbox.pbc[i] && obj1->q[i] > subbox.Lgwbl[i])
        obj1->q[i] -= subbox.Lgwbl[i];
      if (subbox.pbc[i] && obj1->q[i] < 0)
        obj1->q[i] += subbox.Lgwbl[i];
    }
...
double condition_PLC(PRODFLOAT F)
{
  ...
  for (i = 0, condition = 0.0; i < 3; i++)
  {
    ...
    diff1 = q2x(i, &obj1, subbox.pbc[i], (double)subbox.Lgwbl[i], ORDER_FOR_CATALOG) + subbox.stabl[i] - (plc.center[i] - MyGrids[0].GSglobal[i] * replicate[i]);
  ...
  *rho = sqrt(x[0] * x[0] + x[1] * x[1] + x[2] * x[2]);
  if (*rho > 0)
  {
    ...
    *phi = atan2(x[0] * plc.yvers[0] + x[1] * plc.yvers[1] + x[2] * plc.yvers[2],
                 x[0] * plc.xvers[0] + x[1] * plc.xvers[1] + x[2] * plc.xvers[2]) *
           180. / PI;
    if (*phi < 0)
      *phi += 360.;
  }
