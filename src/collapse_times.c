...
int sng_system(double t, const double y[], double f[], void *sng_par)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (j = 0; j < 3; j++)
		{
			...
			{
				sum += (y[j + 6] - y[i + 6]) * ((1. - y[i]) * (1. - y[i]) * (1. + y[i + 3]) - (1. - y[j]) * (1. - y[j]) * (1. + y[j + 3])) /
...
int initialize_collapse_times(int ismooth, int onlycompute)
{
	...
	if (!ismooth)
	{
		...
		else
		{
			...
			{
				ref_interval = ((pow(CT_RANGE_D - CT_DELTA0, 2. - CT_EXPO) + pow(CT_RANGE_D + CT_DELTA0, 2. - CT_EXPO) - 2. * pow(deltaf, 2. - CT_EXPO)) / CT_EXPO / (2. - CT_EXPO) + 2. * deltaf / CT_SQUEEZE) / (CT_NBINS_D - 2.0);
			}
...
inline double interpolate_collapse_time(int ismooth, double l1, double l2, double l3)
{
	...
	ix = (ix >= CT_NBINS_XY - 1) ? CT_NBINS_XY - 2 : (ix < 0) ? 0
															  : ix;
	iy = (iy >= CT_NBINS_XY - 1) ? CT_NBINS_XY - 2 : (iy < 0) ? 0
															  : iy;
	...
	ixstart = (ix == 0) ? 0 : (ix >= CT_NBINS_XY - 2) ? CT_NBINS_XY - 4
													  : ix - 1;
	iystart = (iy == 0) ? 0 : (iy >= CT_NBINS_XY - 2) ? CT_NBINS_XY - 4
													  : iy - 1;
